...
(defn- add-path [sources m]
  (assoc m :path (find-file-in-repo (:file m) sources)))
...
(defn add-source-paths
  ...
  (let [sources (map #(normalize-path % root) sources)]
    (for [ns ns-seq]
      (-> ns
          (assoc :publics (map (partial add-path sources) (:publics ns)))
          (assoc :privates (map (partial add-path sources) (:privates ns)))))))
