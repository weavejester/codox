...
(defn- platforms-menu [project]
  (when (:show-platforms? project)
    (list
      ...
      [:ul.index-link
       [:li.depth-1 (link-to (index-filename :clojure) [:div.inner "Clojure"])]
       [:li.depth-1 (link-to (index-filename :clojure) [:div.inner "ClojureScript"])]])))
...
(defn- header-platforms [project]
  (when (:cross-platform? project)
    (let [{:keys [language]} project]
      [:div#langs
       (if (= language :clojure)
         [:div.lang.current "clj"]
         [:div.lang (link-to (index-filename :clojure) "clj")])

       (if (= language :clojurescript)
         [:div.lang.current "cljs"]
         [:div.lang (link-to (index-filename :clojurescript) "cljs")])])))
   ...
   [:body
    ...
    [:div#content.namespace-index
     ...
     (when (:show-platforms? project)
       (list
         [:h2 "Platforms"]
         [:p "This project includes both Clojure and ClojureScript code, please "
